        ...
        def get_message(self) -> str:
            ...
            message_dict = {
                ...
                self.SYSTEM_NOT_HEALTHY: NotificationMessage(
                    ...
                    _('The system health check detected an issue with one or more services. Please investigate immediately.')
                ),
                self.VULNERABILITY: NotificationMessage(
                    _('Security vulnerability detected'),
                    _('A security vulnerability affecting system components has been detected. Immediate attention required.')
                ),
                ...
                self.DEVICE_ONBOARDING_FAILED: NotificationMessage(
                    _('Device {device} onboarding failed'),
                    _('The device {device} failed onboarding.')
                ),
                ...
                self.INSUFFICIENT_KEY_LENGTH: NotificationMessage(
                    ...
                    _('The certificate {common_name} uses an RSA key size of {spki_key_size} bits, which is less than the recommended 2048 bits.')
                ),
                self.WEAK_ECC_CURVE: NotificationMessage(
                    _('Certificate {common_name} uses a weak ECC curve'),
                    _('The certificate {common_name} is using the {spki_ec_curve} ECC curve, which is no longer recommended.')
                ),
            }
