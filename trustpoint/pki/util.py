...
class ErrorAndWarningSource(Enum):

    BSI = _('BSI Technical Guideline TR-02103')
    RFC5280 = _('RFC5280')
    TRUSTPOINT_CUSTOM = _('Trustpoint specified constraint.')


class CertificateError(Enum):

    # error msg, code, source
    INVALID_VERSION = (_('Certificate version is not v3.'), 'invalid_version', )
    SERIAL_NUMBER_0 = (_('Certificate serial number is 0.'), 'serial_number_0')
    SERIAL_NUMBER_NEGATIVE = (_('Certificate serial number is negative.'), 'serial_number_negative')
...
class CaCertificateError(CertificateError):
...
class CaCertificateWarning(CertificateWarning):
...
class BsiX509CertificateValidator:
    ...
    @property
    def warnings(self) -> list[CertificateWarning]:
    ...
    def is_valid(self) -> bool:
        if self._was_processed:
            ...
            self._run_checks()
            self._is_valid = False if self._errors else self._is_valid = True
...
class BsiCaCertificateValidator(BsiX509CertificateValidator):
    ...
    def _check_subject_key_identifier(self) -> None:
        try:
            ...
            self._add_error(CaCertificateError.EXT_SUBJECT_KEY_ID_MISSING)
            return
    ...
    def _check_key_usage(self) -> None:
        try:
            ...
            self._add_warning(CaCertificateWarning.EXT_KEY_USAGE_MISSING)
            return

        if key_usage.critical is False:
            self._add_warning(CaCertificateWarning.EXT_KEY_USAGE_NON_CRITICAL)

        if not (key_usage.value.key_cert_sign or key_usage.value.crl_sign or key_usage.value.digital_signature):
            self._add_error(CaCertificateError.EXT_KEY_USAGE_FLAG_MISSING)
    ...
    def _check_certificate_policies(self) -> None:
        ...
        if policy_constraints.critical is False:
            self._add_error(CaCertificateError.EXT_POLICY_CONSTRAINTS_NON_CRITICAL)
    ...
    def _check_policy_mappings(self) -> None:
        ...
        if policy_mappings.critical is False:
            self._add_warning(CaCertificateWarning.EXT_POLICY_MAPPING_NON_CRITICAL)
    ...
    def _check_subject_alternative_name(self) -> None:
        ...
        if not self.certificate.subject and subject_alternative_name.critical is False:
            self._add_error(CaCertificateError.EXT_SUBJECT_ALTERNATIVE_NAME_NON_CRITICAL)

        if self.certificate.subject and subject_alternative_name.critical is True:
            self._add_warning(CaCertificateWarning.EXT_SUBJECT_ALTERNATIVE_NAME_CRITICAL)
    ...
    def _check_basic_constraints(self) -> None:
        try:
            ...
            self._add_error(CaCertificateError.EXT_BASIC_CONSTRAINTS_MISSING)
            return

        if basic_constraints.critical is False:
            self._add_error(CaCertificateError.EXT_BASIC_CONSTRAINTS_NON_CRITICAL)

        if basic_constraints.value.ca is False:
            self._add_error(CaCertificateError.EXT_BASIC_CONSTRAINTS_NON_CA)
    ...
    def _check_inhibit_any_policy(self) -> None:
        ...
        if inhibit_any_policy.critical is False:
            self._add_error(CaCertificateError.EXT_INHIBIT_ANY_POLICY_NON_CRITICAL)
