...
#    model=IssuingCa
    ...
    def generate_csr(self,common_name,key_type):
...
#               x509.NameAttribute(x509.NameOID.ORGANIZATIONAL_UNIT_NAME, ORGANIZATIONAL_UNIT),
    ...
    def est_simpleenroll(self,url, data=None, auth=None):
        """
        ...
        response = requests.post(EST_URL, headers=headers, data=data, auth=auth, verify=False)
        
        response.raise_for_status()  # Raise exception for non-200 status codes

        return response

    def est_cacerts(self,url):
        """
        
        """
        EST_URL=url+"/cacerts"
        response = requests.get(EST_URL, verify=False)

        response.raise_for_status()  # Raise exception for non-200 status codes

        return response
    
    def form_valid(self,form: IssuingCaESTForm):
        # Get form data
        est_user_name = form.cleaned_data.get('est_user_name')
        est_password = form.cleaned_data.get('est_password')
        est_url = form.cleaned_data.get('est_url')
        unique_name=form.cleaned_data['unique_name']
        common_name=form.cleaned_data['common_name']
        key_type=form.cleaned_data['key_type']

        # Get CA chain
        response = self.est_cacerts(est_url)
        certs_der = base64.b64decode(response.text)
        certs_chain = pkcs7.load_der_pkcs7_certificates(certs_der)
        
        # Generate private key and CSR
        private_key, csr_der = self.generate_csr(common_name,key_type)
        ...
        response = self.est_simpleenroll(est_url, data, auth)
