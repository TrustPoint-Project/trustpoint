...
def ldevid(request):
    ...
    url_extension = request.path.split('/')[-1]
    ob_process = OnboardingProcess.get_by_url_ext(url_extension)
    if not ob_process or not ob_process.active or request.method != 'POST' or not request.FILES:
    ...
    if ob_process.state == OnboardingProcessState.DEVICE_VALIDATED:
        ob_process.state = OnboardingProcessState.LDEVID_SENT
    return response
