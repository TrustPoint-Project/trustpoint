...
def device_api_dict(device: Device) -> dict:
    ...
    return {
        ...
        'onboarding_status': str(Device.DeviceOnboardingStatus(device.device_onboarding_status).label)
    }
...
def devices(request: HttpRequest):
    ...
    response = []
    for device in qs:
        response.append(device_api_dict(device))
    return response
...
def device(request: HttpRequest, device_id: int):
    ...
    return 200, device_api_dict(device)
